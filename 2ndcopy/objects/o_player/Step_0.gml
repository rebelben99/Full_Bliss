...
grounded = (place_meeting(x,y+1,o_wall2));
//Keybinds::
var _keyLeft = keyboard_check(ord("A"));
var _keyRight = keyboard_check(ord("D"));
var _keyUp = keyboard_check(ord("W"));
...
//Collision w/ terrain
if(place_meeting(x+hSpeed,y,o_wall2)) {
	var _hStep = sign(hSpeed);	
	hSpeed = 0;
	hSpeedFraction = 0;
	while(!place_meeting(x+_hStep,y,o_wall2)) 
		x += _hStep;
	if (state == pState.swing) {
		ropeAngle = point_direction(grappleX,grappleY,x,y);
		ropeAngleVelocity = 0;//bounce
...
if(place_meeting(x,y+vSpeed,o_wall2)) {
	var _vStep = sign(vSpeed);	
	vSpeed = 0;
	vSpeedFraction = 0;
	while(!place_meeting(x,y+_vStep,o_wall2)) 
		y += _vStep;
	if (state == pState.swing) {
		ropeAngle = point_direction(grappleX,grappleY,x,y);
		ropeAngleVelocity = 0;//bounce
